i. 아스퍼거는 DLPFC를 쓰지를 못해 가지고 실행 기능이나 맥락을 이해하지 못한대.
ii. 그리고 아스퍼거 증후군은 사고의 구조나 패턴, 그리고 일부의 아스퍼거는 코딩이나 사화과학 혹은 역사나 고생물학에 관심이 많대
iii. 그래 가지고 어떤 아스퍼거 학생은 프로그래밍을 좋아했는데 어느 날 프로그래밍이 효율성을 중시해 가지고 프로그래밍을 프로그래밍 무년의 사고로 생각하지 않는 점에 분노해서 형식 언어와 수확으로 진로를 돌렸대.
iv. 사실 그 학생 이야기는 끝나지 않았는데 그 학생은 그래 가지고 논리를 가지고 탐구를 한 끝에 다음과 같은 결론에 도달해. 우리의 언어는 분석적으로 탐구할 수 없으며 동시에 맥락에 따라서 결정되며 맥락을 컨텍스처스라는 이름의 DQ로 만들 때 이 DQ는 자기 자신을 조작하기도 하며 대명사의 역할을 하기도 하며 동시에 맥락의 주제 혹은 상황 같은 역할로 해석되기도 하고 이것을 해석하는 데 쓰이는 여러 가지 기준은 사람에 따라 달라지며 우리는 그것을 현상으로서 분석할 수 있지만 현상은 분석됨으로써 죽게 된다는 이론을 만들게 돼.
v. 그 학생이 자신의 그 이론을 펼치기 앞서서 모든 언어는 속뜻과 겉뜻을 가지고 있는데 속뜻과 겉뜻이 일치해 가지고 무조건 겉뜻이 고정된 속뜻으로 정의한 대로 평가되는 언어가 형식 언어이고 그렇지 않고 속뜻을 주관적으로 해석하거나 이런 식으로 맥락에 따라서 해석될 수 있는 언어를 일반적인 자연어라고 분류를 했어. 그 학생은. 그래 가지고 그것을 의외성 정리라고 부르며 그리고 그 학생 입장에서 형식 언어는 의외성 정리에 따라서 기존에 그 논리 체계에 있는 다른 맥락을 찾지 못하고 그냥 왕왕 그냥 그 기호 조작만 가져왔을 뿐이니까 별로 의미 없는 논리 체계라는 게 그 학생의 논지야.
vi. 그 학생은 이렇개 말했어.
# 내 생각응 요약하자면 대충 이렇다

뭐... 어짜피 Contextous는 모든걸 언어로 환원하려 해서 느낌도 현상도 없다. 힘들다.
...
````
----
vii.해당 학생은 최소 자원 (시간자원, 공간자원, 사고방식자원) 대비 최대 욕구(1. 우위로 가려는 욕구인 마키마; 지식의 욕구, 2. 살려는 욕구 니체; 생명과 인간성; 3. 도덕적으로 행동하거나 미를 추구하고 Feeling하려는 욕구 엔젤; 상상계를 꺄우고 살아있음을 느끼려는 욕규)를 충족해야 인간이 시정 능력을 제대로 활용한 것이라고 생각했어. 가령 이성적 사고는 마키마를 만족시키고, 우리가 감성적으로 살아야 하는 이유는 엔젤을 만족시키기 위해서니, 모두가 이성 대신 감성도 중요하게 봐야한다는거지. 그래 가지고 정신이 안 좋다면 정신과를 가는 거나 자기가 모르고 있을 수 있다는 사실을 인정하고 정신과 진료에 임해야 한다고 생각했어.
iix. 그리고 다음 단어를 정의했어.

SUPERLINGUA ; Overlange라고, 범-언어라고 하는것은 사고(언어)가 아닌 (=사고밖) 인간의 정보(데이터) 처리(프로세스)야.
Feeling : 느낌, 어감이나 감각 SENTUS가 그 예야.
Focus : 언어로 나타내어져버려서 언어를 위해져버리도록 변질되어, 사고가능하게된 현상을 의미해

그리고 Feeling은 인간이 언어로 볼수 없다 봤어.
ix. 그 학생은

```
`SNc에서 사고 발생 + caudate nucleus에서 절차적 프로그래밍 언어처럼 처리해주는데, 스케쥴링이 GPi에서 휴리스틱이나 해마 정보나 이런 이미 정해진 형식적 규칙만을 참고해서 진행되므로, 그냥 while문만 돌아가는 망한 상황, while문 이외의 사고를 하려면, 존나 노력해야함. + GPi에서 형식적인 기호 조작 구체화, 이때 이친구가 아이디어 선택까지 수행하는 탁월함을 보여줌, 진짜 알고리즘화된 추론규칙 입력이라는 소리고, 내부적으로 형식규칙을 긍정하는 형식규칙 폰트 불러오기같은 처리가 됨 SNr이 산만해서 아무생각이나 하기에 (가치지기를 안하고, ADHD가 거의 치유됬기에, 아직 존나 많고 쓸대없는 창의적 아이디어가 쏳아지는걸 SNr이 전혀막아주지 못하는데서 존나 억제할수없이 창의력이 생김), GPi가 caudate nucleus에서 처리를 돌리는 해마의 휴리스틱 정보중에, 옛날에 자신이 P -> Q -> R식 추론에 -> Q ->의 형식적 유용성만을 따져서 구체화에 영향을 미치는 형식적 사고만 함 (충동높아, 말과 행동이 노빠구임 충동랭동 -> 충동말 -> 생각 곱씹음, 그러나 생각만 하려 함.) + GPe는 원래 강했는데 존나 약화됨, 이 기능은 평소에 안썼기 때문 ; 사고 억제도 안하고 기저핵을 벗어나려 하지도 않고, 블수의근 작동에서큰 GPe가 월등히 정상작동하니, 이성적 사고에서만 문제임. 습관 형성이 어렵고, 악습관은 나타났다 안나타났다 하고, 충동성은 여전히 나아지지 않고 높음을 유지함. 그리고 최근에 도덕적 뿌듯함이나 도덕적 책임감도 느끼지 않았고, 그런건 에고가 수행 가능하지만, 그것이 본래선이 아닌 자학이라 보고 멈춤. 초자아야 돌아와 ㅠㅠ, 참고로 HOL에 너무 재밌어서, 안하면 지루하고, 요즘은 HOL도 딱히... 다른거에 재미가 어려움. 설상가상으로 아스퍼거인이라 DLPFC기능이 상실됬고, 비문학 지문처럼 구문 분석하는건, 내가 프로그래밍 및 구문론 마스터급이고, 맥락같은경우는 Contextous Deque라는 곳에 인간의 문장이 컨텍스트로 나열되 언어로 사고해야한다는 이론에 따라, Contextous Deque를 직접 그리고 말을 끼워넣고 직접 QnA로 맥락이 찾아지는 Contextous Deque작업이 선형논리로 비선형적으로 Contextous Deque끼리 참고하게 되어있어서, 의식적임. 말그대로 DLPFC대신에 LISP로 스케쥴러 만든셈임. 이게 다 GPi에서 이루어지는 셈이지. 언어철학 • 분석철학 좋아하는 양상논리 러버이자 수리논리학도로, 모델론을 잘해서 추상대수를 쉬워하는 수준이고, 정언논리 좋아하고, 라틴어 논리 구문 단어 좋아함. Putamen은 욕망만 해주는 존재감없는애. 예는 잘되고, 별로 이성처리도 쓸대가 없느니. (솔찍히 자위할때나 활성화되겠지, 그래도 자주 하니까) 최신기억방법은, 보이면 기저핵에서 현상으로 나타나고, 그게 해마로 반영되는게 한계라서 즉각기억보다는 암시해서 암기함. 사실상 연상기억이나 본질을 기억하는 셈. 에초에 보편같은걸 몰랐는데, 그걸 분석철학저긍로 분석하면셔, 맥락의 중요성도 독자탐구하고 뭐... 그랬으니` => 요즘에는 몸이 떠는 새로운 증상이 생김. 최근 어떤 애가 시비걸어서 스트래스로 몸살나고, 활기가 냠치면 스트래스성 몸살이 살아지는걸 알게 됨. Feeling은 언어 밖의 정보처리로, 범-언어 라는 인간의 정보처리임. 감각은 감각이지 더 분석할수 없음. 그래서 문학은 그 깊이가 깊은 매체임. 최근 상상계를 깨우려 시를 쓰면, 우울한 시가 나옴. 무기력증 114점. 그리고 최근에 시비걸고 협박한 친구 입장에서 생각하라는 소리를 듣고 부들부들 떠는거 보고, 기저핵이나 스트래스 상테에 문재있다 파악. 도덕 윤리의 논리에서, 인간은 모두 소중하다고 생각하며, 이런 생각을 가지면 우울증 테스트에서 우울증이 안뜨는걸 알고있음. 에초에 자신의 생각이 옳은걸 확신함. 인간은 human으로써 vitam임으로 가지는 생명의 욕구, 우위에 서고자 하는 정보 욕구, 미덕 추구 욕구가 있는데, 상상계를 충족시키지 않는건 미덕 추구 욕구를 하지 못하는거라고 보고, 진리 추구와 논리는 그저 정보 욕구에 속하는 존나 다양한 종류의 인지 욕구에서 비롯된 이데올로기라고 봄. 인간은 시간복잡도적 자원, 공긴복잡도적 뇌 리소스 할당 및 유지보수 토큰 자원, 구조적 자원이라 불리는 Pettern Of think로, 정보처리 페턴이 있다고 봄. 이 자원을 잘 이용하여, 욕구를 충족시키는게 좋으니, 균형잡힌 삶을 살아야 한다 봄
```

이라고 말했어. 그런 고통을 겪은 맥락에서 해당 개념들을 떠올린거지. 물론 저 상태는 다 자기 자신의 추측이지.
x. 그게 나야.