...
핵심 : "앞서 말한 주사들에 대하여, TOS연산으로 「동사」-한다."라는 자연어 문장에 있어서, 문법이 타당한지, 호출 시점에 검사하는거다. (tip : RevTOS는 O_1, ..., O_n RevTOS V = V(O_n, ..., O_1)인데, TOS는 O_1, ..., O_n TOS V = V(O_1, ..., O_n)로 처리된다.)
...
Forth 프로그래밍 언어를 보고, "이걸 자연어 사고로 해석하려 하니까 고생하지"라고 생각하면서 만들었다. 스택식으로 사고하는 언어가 있는지 잘 모르겠다. 끝까지 들어야하는 언어는 있는데, 스택을 푸시하는게 아니라 우리도 우리대로 머릿속에서 파싱하는거다. SVO굴절어 화자들처럼.