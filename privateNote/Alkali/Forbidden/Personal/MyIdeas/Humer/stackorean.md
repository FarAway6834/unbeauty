...
⚠️ start WARNING : 이 글에서 가장 큰 주의사항입니다. (시작함) ⚠️

물론 읽다가, 토큰어와 TOS에 대해 오해할까봐, 일본어의 경우, 포합어(비토큰어)가 아닌데 띄어쓰기가 없지 않냐는 질문의 반박을 스택일어 문단이 반박을 달아놨고. 스택국어와 스택일어에서 언어 L에 대해, L*이 문자열 타입이라, 모든 단어가 L*이라 타입 검사를 안하기에 TOS에서 타입 시스템을 안씀을 미리 스택국어 문단에 서술해놨다.

N.B. TOS서술에서 타입 시스템 설명을 빼야하는데 빼지 않고 발췌한 이유는, 작성자가 빼기 귀찮아서다. 제거하는게 맞는거다. 글에 사용되지 않는 내용을 정의하는짓이니까.

문법에서 타입 배제주의 : 타입이론은 수학이지 자연어가 아니다. 플라톤주의적인 기괴한 망상증이다. (이 입장도 극단적으로 형식주의적이지. 동형성이 있을순 있으나, 함수와 선형사상이 사상일뿐 실제로는 (X, Y, graph f)와 행렬표현으로 엄연히 다르듯, 타입이론과 수학이 같다는건, HoTT를 긍정하는 급의 병적인 망상이라는건, 나는 철회할 생각이 없다. 내 입장에선 이건 음모론이 아니다. 원자사실과 대응하는 바가 없는건 거짓이므로 펙트임.)

⚠️ end WARNING : 이 글에서 가장 큰 주의사항입니다. (끝남) ⚠️
...
(N.B. 일본어도 본질적으로 발음상 어절이 존재하므로, 발음기호로 옮기는 문법규칙에서 `\b`가 정의됨)
