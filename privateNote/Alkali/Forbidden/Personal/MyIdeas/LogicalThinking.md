# 논리적인 언어활동 (Logical Thinking) 방법 탐구
...
먼저, 겉뜻 의미론과 속뜻 의미론을 분리하여, 기존 의미론의 의미에 대한 해석 방식을 격리해두자.

그 다음. 우리는 진짜 "논리적으로 읽는 법"을 구상해보자. 한번 논리적으로 읽는 삶을 살고싶다.

### DFM++ EXPRESSION과 의외성 정리
...
공리와 증명과정은, 아래 설명으로 대체한다. 정의는 그대로 가져온다.
...
### MGLP : 과거 만든거, 쓰래기임. 아래 요약 문단일 읽고, 이건 읽지 말길.

과거 MGLP라는걸 계획한 적이 있었다. (구림 주의)
...
+ 이 글을 소개하며 얻은 정보
왠지는 모르겠는데 GPT는 CoT → MGLP → CoT체인이 낫다고 말한다. 뭐 인간이라면 자기자신은 지가 재일 잘 알겠지만... 걍 AI 기술에 대해 난 맘에 안드는 타입이라.

근데 솔찍히 로봇 합체하는것 같은 hybrid체인 아이디어라 적어봤다.
````
...
마인드맵을 만들기 위해서,
Step대신에 Node객체를 (그냥 문자열을 담는 일게 순서쌍. first는 값, last는 해설)
추론 규칙 QnA대신에 Edge객체를 (Node포인터를 std::vector로, 해설용 string)
준비해서, Node랑 Edge라는 할당량이 제한된 heap에 할당되는 (할당량 초과시 "too difficult error") C++객체로 관리하면,
GraphRoot객체를 Node퍼인터와 Edge객체로 만들어서

1. std::vector<GraphRoot>들에서 생각이 시작
2. Edge를 생성하면, 결과인 Node는 자명함. (보편적인 인과관계에 의한 추론을 선택; 즉, 대전재 선택)
3. add root • delete root • influence GraphRoot • delete GraphRoot식으로 mash net형태로 된 그래프를 관리해주는 프로그램 필요.

물론 인간의 경우 3번이 자동으로 된다.
그리고, 그래프는 다음 표상이다.
...
이름표를 렌더링 가능한 유클리드 공간상에 점을 찍어서 점에 이름을 노드로, 엣지는 edge.last - edge.first라는벡터를 노드에 접다발로 합쳐서, 벡터에다가, 가중치를 이름으로 렌더링하면.

우리가 그래프를 생각하는 표상이 다이어그램으로 렌더링된다.

그런데, 이 렌더링은 필연적이지 않다. ••• [fact 1]

사실 유클리드 공간도 웃기는 짓이다.

벡터공간의 벡터가 언제부터 좌표라고 생각하는건가? 그건 그냥 튜플일 뿐이지, 그걸 그림이라고 말하는건,

우리의 직관을 형식에게 강요하는거다. ••• [fact 2]

이미 눈치챘겠지만, 그래프 이론의 그래프도 허상이니, 우리의 직관을 형식에게 강요하는거다.

그렇다. 파싱 트리의 기반은, 트리라는 그래프를 가정하는데, 이는 우리의 직관을 형식에게 강요하는거다. ••• [fact 3]

그러므로, 트리나 스타 구조를 기반으로 하는, "모델론의 구조체"와 "수리논리의 생성수형도"도 우리의 직관을 형식에게 강요하는거다. ••• [fact 3]

우리는 자연어로 사고하기에, 직관과 표상을 형식 언어로 옮긴다.

그 이유는 간단하다. 형식 언어는 모국어로 부적합하기에, 우리는 언어(=우리 사고의 한계)가 자연어다.