...
참고로 언젠가 이름을 갈아엎을 생각이다 (최초작성당일 2026.01.20 계획)
참고로, 변수기호에 대한 값-배정으로 치환을 평가하는걸로 암묵적으로 가정한다.

아, 또, 공리계 모델이 "BootstrapperAxiom = BootstrappeeAxiom = T"를 공리로 규정한 셈이 되는거라, 사실상 참을 재정의한것으로, 해당 모델을 긍정하는 전건 이후로 이론이 전개되는거다.

`SI ⊨ 공리계, ERGO 이후의 서술`구조고, 이것은 언어를 이용하겠다는 선언이고, 현재 사용중인 문법규칙의 추가다.

상태가 아예달라진거다. 공리계에 Bang `!` (Of-Course연산)을 취하고, 이후 선형함축을 취했다고 볼수도 있다.

그러므로, 외부의 시선으로 나가는 순간, lang"서술"을 다루는건데, lang"서술"은, 현재 형식 언어에다가 `SI ⊨ 공리계, ERGO `를 추가해서, 문법 규칙을 추가한 언어 lang에 대한 서술이다.

그래서 공리 추가가 가능한거다. 아예 논의 영역을 옮겼으니까!

에초에, 저게 긍정된다는거 자체부터, 언어가 달라졌다는 근거다!

ZF에서 AC는 긍정도 부정도 안된다!

형식언어 lang의 문법규칙 P가 P = 기존 ∪ {NewAxiom ⟶ T}로 갱신됬기 때문에, 형식언어 lang의 문법 G의 컴파일 이전 문자셋 lang에 대해, lang* 가 도매인인 자유 모노이드에 대한 형식적 조작의 대수가 되는거다.
