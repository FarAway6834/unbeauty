# JudgmentDayOfInduction

예외논리는 예외 발생이 가능하다.

현실새계 a와 이론새계(우리 언어 내부) x에 대해,
무한한 새계가 존재강제구조로 나열되어있을때 (존재강제구조가 알레프-널 갯수)

그러한 상황에서 이 모델이 알려주는건, 새상 a에서 예외가 발생하는건 변칙성이고, 상위공간인 h에서 규칙성을 뛰는건 규칙성, 즉 이론에 대한 만족이다.

우리는 변칙성과 규칙성으로 인지하는것 외에 새상을 이해할 수 없다. 인식은 필연적으로 연역적-방법을-통한-추론으로는 새상을 이해할수 없는 구조이다.

즉, 저러한 예외 구조가 과학적 이론이라면, 이를 변칙성-구조라고 하며,

변칙성-구조가 발견되는 순간, 귀납이 인식론적 종말을 맞이했다고 봐도 될것이다.

변칙성-구조에 따라서, 변칙성-구조가 설명하는 대상은, 우리가 연역적인 인식을 통해서 합리화할수 없기 때문이다.

즉, 새상이 합리화할수 없으니 귀납을 쓴다는 쓰디쓴 펙트를 인정하는 셈이 된다.

왜 귀납의 심판일이냐면, 나는 연역을 좋아한다. (감정적인 이유지, 귀납적인 이유가 아니다. 귀납이나 연역이나 그 논리적인 힘은 "옳다"는 판단에서 오기에, 동일하다고 나는 주장한다)

지금보니 이름이 좀 편향적이다.

에초에 변칙성-구조 자체가 페러다임 시프트가 왜 인식과정인지를 보여주는, 즉, 과학 이론 자체가 인식과정이 되는, 정지문제를 푸는것과 같은 상태를 초래하기 때문이다.
