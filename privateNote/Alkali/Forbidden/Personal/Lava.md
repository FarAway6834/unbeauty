...
서술 편의를 위한 모델이나 표기법을 도입한다.

Dragon Book에서 표기법을 순전히 편의를 위해 도입하듯이 말이다.

### 서술 편의를 위한 모델들

모델이나 표기법 중 모델들을 나열하겠다.

#### TRS

TupleRequirsivlySplit모델의 정의다 (걍 first랑 last를 만드는거다)
...
서술 편의를 위한 표기법들을 도입한다.

모델이나 표기법 중 표기법들을 나열하겠다.
...
함수 정의 두줄씩 쓰는거에 질려서 걍 익명함수로 표기하고싶었다. 이 노트(Lava System)는 고1이엇는지 고2초에 썼던것 같은데, 계속 편집하며 양이 늘어낫다. 하나하나 쓰는게 질릴정도로 썻다. 26.04.15기점으로는 익명함수를 애용할듯하다.
...
1. [Endofunctor Type System](https://faraway6834.github.io/2026/06/12/PostingETS.html)

과거 1066~1511줄에 위치햤던 Endofunctor Type System은, 저 링크 "https://faraway6834.github.io/2026/06/12/PostingETS.html"로 추방당했으니, 알아서 링크에 들어가서 보도록 하라.
...
어짜피 이거, 심볼로써 무슨 의미인지 알려면, 그 순술어를 알아보는수밖에 없다. 해설? 귀찮다. 난 더이상, 4월의 그 고 2 (최초 작성당시 본인)만큼 친절하고 성실하지 않다. ㅋ
...
근데 실은, 좌항등원과 우항등원의 정의는, 국소적 항등원의 "국소 to 전역화(「국소 to 전역 성질분석」문단의 국소 to 전역화 문단 참고)"를 통해 구성된다.

해당 구성이 이 문단에서 감춘 진실이지만, 굳이 보지 않아도, 이 문단 선에서 이해 가능한지라, 국소 to 전역화라는 이름으로  "은폐를 통한 추상화 (추상화 과정에서 정보를 은페하여, 추상화의 효능(간단해지는것)을 얻는것.)"가 된 셈이다.

참고로, 이건 어디까지나, 전역적 항등원 e고, 국소적 항등원 E가 어떻게 전칭양화되어 전역적 항등원 e가 되는지는, 「국소 to 전역 성질분석」문단의 항등원 항목에 있다.
